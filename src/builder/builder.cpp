...
void Builder::border_cell(Cell& cell) const
{
	...
	if (cell.position_.x - rad < protozoa_space.position.x)
	{
		cell.position_.x = protozoa_space.position.x + rad;
		//velocity.x *= -1.f;
	}

	else if (cell.position_.x + rad > protozoa_space.position.x + protozoa_space.size.x)
	{
		cell.position_.x = protozoa_space.position.x + protozoa_space.size.x - rad;
		//velocity.x *= -1.f;
	}

	if (cell.position_.y - rad < protozoa_space.position.y)
	{
		cell.position_.y = protozoa_space.position.y + rad;
		//velocity.y *= -1.f;
	}

	else if (cell.position_.y + rad > protozoa_space.position.y + protozoa_space.size.y)
	{
		cell.position_.y = protozoa_space.position.y + protozoa_space.size.y - rad;
		//velocity.y *= -1.f;
	}
...
{
	const sf::Vector2f bounds_pos = bounds.position;
	const sf::Vector2f bounds_size = bounds.size;
...
{
	const sf::Rect<float> rect = { {bounds.position.x + button_buffer.x, bounds.position.y + bounds.size.y - button_size.y - button_buffer.y},
...
void Builder::init_debug_button()
{
	...
	rect.position.x += rect.size.x + button_spacing;
...
void Builder::init_bounds()
{
	...
	protozoa_space.position.y -= 50.f;

	m_bounds_render_.setPosition(bounds.position);
	m_bounds_render_.setSize(bounds.size);
...
{
	m_protozoa_bounds_.radius = protozoa_space.size.x / 1.3f;
