...
public:
	...
	{
		const sf::Vector2f border_pos = m_border_.position;
		const sf::Vector2f border_size = m_border_.size;
...
private:
	...
	{
		m_render_border_.setPosition(m_border_.position);
		m_render_border_.setSize(m_border_.size);
	...
	void initialize_button()
	{
		...
		const sf::Rect<float> rect = {
			{m_border_.position.x + m_border_.size.x - buffer.x - size.x,
			m_border_.position.y + m_border_.size.y - buffer.y - size.y},
			{size.x, size.y} };
	...
	void initialize_node_positions()
	{
		...
		const float layer_sep_x = m_border_.size.x / (shape.size() + 1);
		std::vector<float> x_positions(shape.size());

		for (int i = 0; i < shape.size(); ++i)
		{
			x_positions[i] = m_border_.position.x + layer_sep_x * (i + 1);
		}
		...
		for (int layer_idx = 0; layer_idx < neural_network.size(); ++layer_idx)
		{
			...
			const float layer_sep_y = m_border_.size.y / (num_nodes + 1);

			for (int node_idx = 0; node_idx < num_nodes; ++node_idx)
			{
				const sf::Vector2f position(
					x_positions[layer_idx],
					m_border_.position.y + layer_sep_y * (node_idx + 1)
				);
