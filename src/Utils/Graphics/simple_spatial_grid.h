...
{
public:
	explicit SimpleSpatialGrid(const sf::FloatRect screen_size = {}) : m_screenSize(screen_size), text{font}
	{
	...
	cell_idx inline add_object(const float x, const float y, const size_t obj_id)
	{
		...
		if (m_screenSize.size.x == 0.f || m_screenSize.size.y == 0)
		{
...
private:
	...
	void init_graphics()
	{
		...
		m_screenSize.position.x -= resize;
		m_screenSize.position.y -= resize;
		m_screenSize.size.x += resize;
		m_screenSize.size.y += resize;

		m_cellSize = { m_screenSize.size.x / static_cast<float>(CellsX),
						  m_screenSize.size.y / static_cast<float>(CellsY) };
