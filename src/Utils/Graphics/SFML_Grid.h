...
public:
		...
		: window_(window), world_dimensions_(world_dimensions), max_nest_level_(max_nest_level)
	{
		...
		const float cell_size = world_dimensions.size.x / cells_x;
		...
		for (size_t x = 0; x < cells_x; x++)
		{
			...
			vertices[counter + 1].position = { world_dimensions.position.x + posX, world_dimensions.position.y + world_dimensions.size.y };
			counter += 2;
		}
		...
		for (size_t y = 0; y < cells_y; y++)
		{
			...
			vertices[counter + 1].position = { world_dimensions.position.x + world_dimensions.size.x, world_dimensions.position.y + posY };
			counter += 2;
		}
