...
private:
        ...
        const sf::FloatRect& screen)
    {
        ...
        float nx = (x - world.position.x) / world.size.x;
        float ny = (y - world.position.y) / world.size.y;

        return {
            screen.position.x + nx * screen.size.x,
            screen.position.y + (1.f - ny) * screen.size.y
        };
    }
    ...
    {
        sf::VertexArray wave(sf::PrimitiveType::LineStrip, m_settings.sample_count);
    ...
    void draw_labels(sf::RenderTarget& target, const sf::FloatRect& screen) const
    {
        // X label
        sf::Text x_text{m_font};
        x_text.setString(m_settings.x_label);
        x_text.setCharacterSize(16);
        x_text.setFillColor(sf::Color::White);

        const auto xb = x_text.getLocalBounds(); // { position, size }
        x_text.setOrigin({ xb.position.x + xb.size.x * 0.5f, xb.position.y });
        x_text.setPosition({
            screen.position.x + screen.size.x * 0.5f,
            screen.position.y + screen.size.y + 30.f
            });

        target.draw(x_text);

        // Y label
        sf::Text y_text{ m_font };
        y_text.setString(m_settings.y_label);
        y_text.setCharacterSize(16);
        ...
        y_text.setRotation(sf::degrees(-90.f));

        const auto yb = y_text.getLocalBounds();
        y_text.setOrigin({ yb.position.x + yb.size.x * 0.5f, yb.position.y });
        y_text.setPosition({
            screen.position.x - 40.f,
            screen.position.y + screen.size.y * 0.5f
            });
