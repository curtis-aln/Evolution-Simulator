...
{
	// SFML 3.x change: sf::Lines -> sf::PrimitiveType::Lines
...
inline void draw_protozoa_bounding_box(const sf::Rect<float>& rect, sf::RenderWindow& window, const sf::RenderStates& render_states = sf::RenderStates())
{
	...
	draw_protozoa_bounding_box(pos, pos + size, window, render_states);
}
...
inline sf::Rect<float> convert_coordinates(const sf::Vector2f& v1, const sf::Vector2f& v2) {
	...
	// SFML 3.x change: Rect constructor takes position and size
	return sf::Rect<float>(sf::Vector2f(x, y), sf::Vector2f(width, height));
}
...
{
	// SFML 3.x change: sf::Triangles -> sf::PrimitiveType::Triangles
...
inline sf::VertexArray make_circle(const float radius, const sf::Vector2f center, const unsigned resolution = 100) {
	// SFML 3.x change: sf::LinesStrip -> sf::PrimitiveType::LineStrip
	...
	const float thickness = {}, const float outline_thickness = {}, const sf::Color& fill_color = {}, const sf::Color& outline_color = {})
{
	...
	// SFML 3.x change: setRotation takes sf::Angle
...
inline sf::Color interpolate_colors(const sf::Color& color_a, const sf::Color& color_b, float x) {
	...
	const std::uint8_t r = static_cast<std::uint8_t>(color_a.r + t * (color_b.r - color_a.r));
	const std::uint8_t g = static_cast<std::uint8_t>(color_a.g + t * (color_b.g - color_a.g));
	const std::uint8_t b = static_cast<std::uint8_t>(color_a.b + t * (color_b.b - color_a.b));
