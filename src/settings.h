...
{
	static constexpr std::uint8_t transparency = 220;
...
public:
	inline static sf::Vector2f get_window_dimensions()
	{
		...
		// SFML 3.x change: size is now a member variable, not a function
		sf::Vector2u size_int = vid.size;
...
public:
	...
	inline static const sf::FloatRect protozoa_graph_bounds = 
		sf::FloatRect(sf::Vector2f(line_graph_buffer/2, line_graph_y), 
		              sf::Vector2f(line_graph_width, line_graph_height));

	inline static const sf::FloatRect food_graph_bounds = 
		sf::FloatRect(sf::Vector2f(line_graph_buffer * 1.5f + line_graph_width, line_graph_y), 
		              sf::Vector2f(line_graph_width, line_graph_height));

	inline static const sf::FloatRect text_renderer_bounds = 
		sf::FloatRect(sf::Vector2f(100, 100), sf::Vector2f(500, 500));
...
{
	// SFML 3.x change: FloatRect constructor
	inline static const sf::FloatRect bounds = 
		sf::FloatRect(sf::Vector2f(200, 200), sf::Vector2f(2100, 1350));
...
struct FoodSettings
{
	...
	static constexpr std::uint8_t transparency = 200;
